...
interface Availability { id: number; name: string; timezone: string; is_default: number; days: AvailDay[]; }
...
export default function AvailabilityPage() {
  ...
  const [activeAvail, setActiveAvail] = useState<Availability | null>(null);
  ...
  async function fetchAll() {
    ...
    if (availRes.ok) {
      ...
      setAvails(data);
      if (data.length > 0) setActiveAvail(data[0]);
    }
  ...
  function toggleDay(dayIdx: number) {
    if (!activeAvail) return;
    setActiveAvail({ ...activeAvail, days: activeAvail.days.map(d => d.day_of_week === dayIdx ? { ...d, is_enabled: d.is_enabled ? 0 : 1 } : d) });
  }

  function updateTime(dayIdx: number, field: 'start_time' | 'end_time', value: string) {
    if (!activeAvail) return;
    setActiveAvail({ ...activeAvail, days: activeAvail.days.map(d => d.day_of_week === dayIdx ? { ...d, [field]: value } : d) });
  }
  ...
  async function save() {
    ...
    const res = await fetch('/api/availability', {
      ...
      body: JSON.stringify({ id: activeAvail.id, name: activeAvail.name, timezone: activeAvail.timezone, days: activeAvail.days })
    });
    if (res.ok) {
  ...
  async function createNew() {
    ...
    if (res.ok) {
      showToast('New schedule created!', 'success');
  ...
  async function deleteSchedule(id: number) {
    if (avails.length <= 1) {
      showToast('Cannot delete last schedule', 'error');
      return;
    }
    // Optional: backend DELETE
    showToast('Deletion coming soon', 'info');
  }
  ...
  if (editingSchedule && activeAvail) {
    return (
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 20, fontWeight: 700, color: 'var(--text-primary)' }}>
                <input
                  value={activeAvail.name}
                  onChange={e => setActiveAvail({ ...activeAvail, name: e.target.value })}
                  style={{ background: 'transparent', border: 'none', color: 'inherit', font: 'inherit', borderBottom: '1px dashed var(--border)', outline: 'none' }}
                />
          ...
          <div style={{ flex: '1 1 600px', display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <div style={{ border: '1px solid var(--border)', borderRadius: 12, background: 'var(--bg-card)', overflow: 'hidden' }}>
              <div style={{ padding: '24px 32px' }}>
                ...
                <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4, marginBottom: 16 }}>Add dates when your availability changes.</div>
          ...
          <div style={{ flex: '1 1 320px', display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div>
              ...
              <select
                ...
                value={activeAvail.timezone} onChange={e => setActiveAvail({ ...activeAvail, timezone: e.target.value })}
              >
          ...
          <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && setShowOverrideForm(false)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
            <div className="modal" style={{ maxWidth: 440, background: 'var(--bg-card)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden', width: '100%', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.5)' }}>
              ...
              <div className="modal-body" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {[{ val: true, label: '🚫 Block' }, { val: false, label: '✏️ Hours' }].map(opt => (
                      <button key={String(opt.val)} onClick={() => setOverrideForm(f => ({ ...f, is_blocked: opt.val }))} style={{ flex: 1, padding: '10px', borderRadius: 8, fontSize: 13, fontWeight: 500, border: `1.5px solid ${overrideForm.is_blocked === opt.val ? 'var(--text-primary)' : 'var(--border)'}`, background: overrideForm.is_blocked === opt.val ? 'var(--text-primary)' : 'transparent', color: overrideForm.is_blocked === opt.val ? 'var(--bg)' : 'var(--text-primary)', cursor: 'pointer' }}>
                        {opt.label}
                      </button>
                    ))}
                  </div>
                ...
                <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  <input style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--text-primary)', borderRadius: 6, padding: '8px 12px', fontSize: 14 }} placeholder="e.g. Public holiday…" value={overrideForm.reason} onChange={e => setOverrideForm(f => ({ ...f, reason: e.target.value }))} />
                </div>
              </div>
              <div className="modal-footer" style={{ borderTop: '1px solid var(--border)', padding: '16px 24px', display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
                <button style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text-primary)', padding: '8px 16px', fontSize: 14, cursor: 'pointer' }} onClick={() => setShowOverrideForm(false)}>Cancel</button>
                <button style={{ background: 'var(--text-primary)', border: 'none', borderRadius: 6, color: 'var(--bg)', padding: '8px 16px', fontSize: 14, cursor: 'pointer' }} onClick={addOverride}>Add Override</button>
              </div>
            </div>
          </div>
        )}
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', color: 'var(--text-color, inherit)', paddingBottom: 60 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '32px 32px 24px', flexWrap: 'wrap', gap: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button
            ...
            style={{ padding: '6px 12px', background: 'var(--text-primary)', color: 'var(--bg)', border: 'none', borderRadius: 6, display: 'flex', alignItems: 'center', fontWeight: 500, fontSize: 13, cursor: 'pointer' }}
          >
      ...
      <div style={{ padding: '0 32px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        {avails.map(avail => (
          <div
            ...
            onClick={() => { setActiveAvail(avail); setEditingSchedule(true); }}
            onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--text-muted)'}
            onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}
          >
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <div style={{ fontWeight: 600, fontSize: 15, color: 'var(--text-primary)' }}>{avail.name}</div>
                {avail.is_default === 1 && <div style={{ background: 'var(--border)', color: 'var(--text-secondary)', padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600 }}>Default</div>}
              </div>
      ...
      {/* New Schedule Modal */}
      {showNewModal && (
        <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && setShowNewModal(false)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div className="modal" style={{ maxWidth: 400, background: 'var(--bg-card)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden', width: '100%', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.5)' }}>
            <div className="modal-header" style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)' }}>
              <h2 style={{ fontSize: 18, fontWeight: 600, margin: 0 }}>Create a new schedule</h2>
              <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>Give your schedule a name like 'Working Hours'</p>
            </div>
            <div className="modal-body" style={{ padding: '24px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ fontSize: 13, fontWeight: 500 }}>Name *</label>
                <input
                  placeholder="Weekend sync, Support etc."
                  autoFocus
                  style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--text-primary)', borderRadius: 6, padding: '10px 12px', fontSize: 14, outline: 'none' }}
            ...
            <div className="modal-footer" style={{ borderTop: '1px solid var(--border)', padding: '16px 24px', display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              <button style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text-primary)', padding: '8px 16px', fontSize: 14, cursor: 'pointer' }} onClick={() => setShowNewModal(false)}>Cancel</button>
              <button style={{ background: 'var(--text-primary)', border: 'none', borderRadius: 6, color: 'var(--bg)', padding: '8px 16px', fontSize: 14, cursor: 'pointer', fontWeight: 600 }} onClick={createNew}>Continue</button>
            </div>
          </div>
        </div>
      )}
